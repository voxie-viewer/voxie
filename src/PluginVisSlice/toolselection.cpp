...
{
    if(mask != nullptr) {
    voxie::data::SliceImage sliceImg = sv->sliceImage();
    ...
    if (rectangleActive)
    {
        ...
        //if (firstValue)
        //{
            sliceImg.imagePoint2PlanePoint(tempPos.x(), tempPos.y(), QSize(sv->canvasWidth(), sv->canvasHeight()), sv->currentPlaneArea(), startRect, true);
           // firstValue = false;
            previewActive = true;
            mousePressed = true;
        /*} else {
            ...
            qDebug()<< tempPos;
            QPointF endRect;
            sliceImg.imagePoint2PlanePoint(tempPos.x(), tempPos.y(), QSize(sv->canvasWidth(), sv->canvasHeight()), sv->currentPlaneArea(), endRect);
    ...
    if (polygonActive)
    {
        ...
        QPointF polyPoint;
        sliceImg.imagePoint2PlanePoint(tempPos.x(), tempPos.y(), QSize(sv->canvasWidth(), sv->canvasHeight()), sv->currentPlaneArea(), polyPoint, true);
    ...
    if(ellipseActive)
    {
        //if (firstValue)
        //{
            sliceImg.imagePoint2PlanePoint(tempPos.x(), tempPos.y(), QSize(sv->canvasWidth(), sv->canvasHeight()), sv->currentPlaneArea(), middlePointEllipse, true);
          //  firstValue = false;
            previewActive = true;
            mousePressed = true;
        /*} else {
            QPointF radius;
            sliceImg.imagePoint2PlanePoint(tempPos.x(), tempPos.y(), QSize(sv->canvasWidth(), sv->canvasHeight()), sv->currentPlaneArea(), radius);
...
void ToolSelection::toolMouseReleaseEvent(QMouseEvent *e)
{
    ...
    voxie::data::SliceImage sliceImg = sv->sliceImage();
    QPointF tempEnd;
    sliceImg.imagePoint2PlanePoint(e->pos().x(), e->pos().y(), QSize(sv->canvasWidth(), sv->canvasHeight()), sv->currentPlaneArea(), tempEnd, true);
    if (rectangleActive)
    {
...
QPainterPath ToolSelection::planeToImage(QPainterPath path)
{
    ...
    //qreal x = ((path.boundingRect().left() - sv->currentPlaneArea().left())/(sv->currentPlaneArea().width())) * sv->canvasWidth();
    //qreal y = ((path.boundingRect().top() - sv->currentPlaneArea().top())/(sv->currentPlaneArea().height())) * sv->canvasHeight();

    //qDebug() << path.boundingRect().left() << path.boundingRect().top();
    //qDebug() << x << y;
    //QTransform translate(1, 0, 0, 1, -sv->currentPlaneArea().left(), -sv->currentPlaneArea().top());
    QTransform translate(1, 0, 0, 1, -sv->currentPlaneArea().left(), -sv->currentPlaneArea().bottom());
    qreal scaleX = sv->canvasWidth() / sv->currentPlaneArea().width();
    qreal scaleY = sv->canvasHeight() / sv->currentPlaneArea().height();
