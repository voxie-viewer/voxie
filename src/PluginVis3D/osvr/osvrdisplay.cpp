...
class Qt5ToolkitImpl {
  ...
  public:
    Qt5ToolkitImpl(QGLWidget* shareWidget, QObject* objectToDelete) {
    ...
    bool addOpenGLContext(const OSVR_OpenGLContextParams* p) {
        ...
        layout->setMargin(0);
        layout->setSpacing(0);
        auto gl = new QGLWidget(nullptr, glwidgets.size() ? glwidgets.at(0) : shareWidget);
        layout->addWidget(gl);
        if (!gl->isSharing()) {
            delete dialog;
        ...
        if (objectToDelete) {
            //QObject::connect(dialog, &QObject::destroyed, objectToDelete, &QObject::deleteLater);
            QObject::connect(dialog, &QDialog::finished, objectToDelete, &QObject::deleteLater);
        }
...
OsvrDisplay::OsvrDisplay(QOpenGLContext* context, QGLWidget* shareWidget, QObject* parent) : QObject(parent), context(context) {
    ...
    toolkit = new Qt5ToolkitImpl(shareWidget, this);
    ...
    renderManager = QSharedPointer<osvr::renderkit::RenderManager>
        (osvr::renderkit::createRenderManager(osvrContext->get(), "OpenGL", myLibrary));
    if (!renderManager || !renderManager->doingOkay())
        throw voxie::scripting::ScriptingException("de.uni_stuttgart.Voxie.OSVR.Error", "Could not create RenderManager");

    auto ret = renderManager->OpenDisplay();
    if (ret.status == osvr::renderkit::RenderManager::OpenStatus::FAILURE)
        throw voxie::scripting::ScriptingException("de.uni_stuttgart.Voxie.OSVR.Error", "Could not open display");

    toolkit->makeCurrent(0);

    if (!initializeOpenGLFunctions())
        throw voxie::scripting::ScriptingException("de.uni_stuttgart.Voxie.OSVR.Error", "Failed to initialize OpenGL functions");
    ...
    if (!renderManager->RegisterRenderBuffers(colorBuffers)) {
        throw voxie::scripting::ScriptingException("de.uni_stuttgart.Voxie.OSVR.Error", "RegisterRenderBuffers() returned false");
    }
...
OsvrDisplay::~OsvrDisplay() {
    for (const auto& dialog : toolkit->getDialogs())
        dialog->deleteLater();
}
...
void OsvrDisplay::updateDisplay() {
    ...
    if (!renderManager->PresentRenderBuffers(colorBuffers, renderInfos)) {
        //qDebug() << "PresentRenderBuffers() returned false, maybe because it was asked to quit";
        timer->stop();
        deleteLater();
        return;
    }
