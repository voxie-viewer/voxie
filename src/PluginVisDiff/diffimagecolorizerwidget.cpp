#include "diffimagecolorizerwidget.hpp"

#include <PluginVisDiff/colorizerworker.hpp>
#include <PluginVisDiff/diffvisualizer.hpp>
#include <PluginVisDiff/makehandbutton.hpp>
...
DiffImageColorizerWidget::DiffImageColorizerWidget(QWidget *parent) :
    QWidget(parent),
    ...
    requestColorizer(false)
{
    ...
    this->currentValueInput->setValidator(new NumberValidator());//new MyValidator(0.0, 1.0, 3, currentValueInput));
...
DiffImageColorizerWidget::~DiffImageColorizerWidget()
{
    //delete filter;
}

void DiffImageColorizerWidget::doColorizer(voxie::data::FloatImage image)
{
    if(!requestColorizer) {
...
void DiffImageColorizerWidget::updateButtons() {
    //qDebug() << "DISABLED";
    ...
    for (QPair<float, QRgb> mapping : colorizerInstance.getMappings()) {
        ...
        inValue->setValidator(new NumberValidator());//MyValidator(0.0, 1.0, 3, inValue));
        ...
        connect(this, &DiffImageColorizerWidget::deleteOldButtons, row, &QHBoxLayout::deleteLater);
        connect(this, &DiffImageColorizerWidget::deleteOldButtons, inValue, &QLineEdit::hide);
        connect(this, &DiffImageColorizerWidget::deleteOldButtons, inValue, &QLineEdit::deleteLater);
        connect(this, &DiffImageColorizerWidget::deleteOldButtons, removeMappingButton, &MakeHandButton::hide);
        connect(this, &DiffImageColorizerWidget::deleteOldButtons, removeMappingButton, &MakeHandButton::deleteLater);
        connect(this, &DiffImageColorizerWidget::deleteOldButtons, colorWidgetRow, &MakeHandButton::hide);
        connect(this, &DiffImageColorizerWidget::deleteOldButtons, colorWidgetRow, &MakeHandButton::deleteLater);
    }
...
void DiffImageColorizerWidget::updateMapping(float old, float in, QRgb out) {
...
void DiffImageColorizerWidget::removeMapping(float in)
{
...
void DiffImageColorizerWidget::addMapping(float in, QRgb out)
{
    //qDebug() << "addMapping";
    colorizerInstance.putMapping(in, out);
    emit gradientChanged(colorizerInstance.getColorMap());
