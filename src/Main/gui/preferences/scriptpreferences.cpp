...
ScriptPreferences::ScriptPreferences(QWidget *parent) :
	QWidget(parent)
{
	...
	QVBoxLayout *mainlayout = new QVBoxLayout();
	{
		...
		{
			QAction *add = toolbar->addAction(QIcon(":/icons/script--plus.png"), "Add scripting tool.");
			connect(add, &QAction::triggered, this, &ScriptPreferences::addNew);

			QAction *minus = toolbar->addAction(QIcon(":/icons/script--minus.png"), "Remove scripting tool.");
			connect(minus, &QAction::triggered, this, &ScriptPreferences::removeCurrent);

			QAction *dup = toolbar->addAction(QIcon(":/icons/scripts.png"), "Duplicate scripting tool.");
			connect(dup, &QAction::triggered, this, &ScriptPreferences::duplicateCurrent);
		}
		...
		QHBoxLayout *listlayout = new QHBoxLayout();
		{
			...
			{
				this->updateList();
				connect(list, &QListWidget::itemSelectionChanged, this, &ScriptPreferences::updateData);
			}
			...
			QVBoxLayout *rightlayout = new QVBoxLayout();
			{
				...
				{
					spaceLayout->addSpacerItem(new QSpacerItem(0, 0, QSizePolicy::Expanding));
					QPushButton *applyButton = new QPushButton("Apply");
					connect(applyButton, &QPushButton::clicked, this, &ScriptPreferences::storeData);
...
void ScriptPreferences::storeData()
{
	...
	this->scriptingTools.insert(
		key,
		{
			this->extensionEdit->text(),
			this->executableEdit->text(),
			this->argsEdit->text()
		});
...
void ScriptPreferences::save()
{
	...
	for(const QString &key : this->scriptingTools.keys())
	{
		...
		Root::instance()->settings()->setValue("extension", this->scriptingTools[key].extension);
		Root::instance()->settings()->setValue("executable", this->scriptingTools[key].executable);
		Root::instance()->settings()->setValue("arguments", this->scriptingTools[key].arguments);
	}
	Root::instance()->settings()->endArray();
	Root::instance()->settings()->endGroup();
}
