 ...
 * cl::Context.
 */
class VOXIECORESHARED_EXPORT CLInstance: public QObject
{
    Q_OBJECT
private:
...
public:
     ...
     */
    static CLInstance* getDefaultInstance() NOEXCEPT
     ...
     * @throws CLException (CL_INVALID_CONTEXT) if this instances context is invalid
     */
     ...
     * @throws CLException (CL_INVALID_CONTEXT) if this instances context is invalid
     */
    QVector<cl::Device> getDevices(cl_device_type type = CL_DEVICE_TYPE_ALL) const EXCEPT;

    /**
     * @return device of preferred type from instances context, if instance
     * contains no device of preferred type the first device returned by opencl::getDevices(context)
     * will be returned.
     ...
     * contains no device of preferred type the commandQueue for the first device of this instance
     * is returned.
     * @param preferredDevice type of preferred device
     * @throws CLException (CL_INVALID_CONTEXT) if this instances context is invalid
     */
    cl::CommandQueue getCommandQueue(cl_device_type preferredDevice = CL_DEVICE_TYPE_GPU) const EXCEPT;

    /**
     * @return commandQueue for given device, if device is not part of this instance
     * an invalid command queue will be returned.
     ...
     * contains no device of preferred type a commandQueue for the instances first device
     * will be created and returned
     * @param preferredDevice preferred device type
     * @throws CLException. See opencl::createCommandQueue() for details about
     * throw causes.
     */
    cl::CommandQueue createCommandQueue(cl_device_type preferredDevice = CL_DEVICE_TYPE_GPU) const EXCEPT;
    ...
    /**
     * Creates program from source string and builds it for all devices of this instance.
     ...
     * @throws CLException
     */
    cl::Program createProgram(const QString& source, const QString& buildOptions, const QString& id = QString()) EXCEPT;
     ...
     */
    cl::Program createProgramFromFile(const QString &filename, const QString& buildOptions, const QString& id = QString()) EXCEPT;

    /**
     * @return Program for given id, if no program is stored with this id an invalid
     * cl::Program is returned.
     * @param programId
     */
     ...
     */
    cl::Kernel getKernel(const QString& programId, const QString& kernelName) const EXCEPT;

    /**
     * Enqueues NDRange Kernel on given commandQeueue with given global works size,
     * without offset and local work size determinded by by the openCl implementation.
     ...
     * @throws CLException (CL_INVALID_PROGRAM_EXECUTABLE, CL_INVALID_COMMAND_QUEUE,
     * CL_INVALID_KERNEL, CL_INVALID_CONTEXT, CL_INVALID_KERNEL_ARGS,
     * CL_INVALID_WORK_DIMENSION, CL_INVALID_GLOBAL_WORK_SIZE,
     * CL_INVALID_WORK_GROUP_SIZE if __attribute__((reqd_work_group_size(X, Y, Z)))
     * qualifier is used in program for kernel, CL_INVALID_IMAGE_SIZE
     * CL_OUT_OF_RESOURCES, CL_MEM_OBJECT_ALLOCATION_FAILURE, CL_OUT_OF_HOST_MEMORY)
     * @see clEnqueueNDRangeKernel()
     */
    static void executeKernel(const cl::Kernel& kernel, const cl::NDRange &globalWorkSize, const cl::CommandQueue& commandQueue, bool blocking = true) EXCEPT;
     ...
     * @param blocking blocking if true, method will return when kernel finished executing
     * otherwise method will return emmediately
     * @throws CLException see CLInstance::executeKernel()
     */
    void executeKernel(const cl::Kernel& kernel, const cl::NDRange &globalWorkSize, cl_device_type preferredDeviceType = CL_DEVICE_TYPE_GPU, bool blocking = true) const EXCEPT;
    ...
    /**
     * creates cl::Buffer of given byteSize and fills it with hostMemory if provided
     * @return buffer
     * @param byteSize size of buffer in bytes
     * @param hostMemory pointer to hostMemory returned buffer shall be filled with
     * @throws CLException
     */
    cl::Buffer createBuffer(size_t byteSize, void* hostMemory = nullptr) const EXCEPT;
    /**
    ...
    QVector<T> readBuffer(const cl::Buffer& buffer) const EXCEPT
    {
        ...
        size_t numElements = (bufferSize % elementSize == 0) ? bufferSize / elementSize : (bufferSize / elementSize) +1;
    ...
    /**
     * creates image3D of given format and dimensions filled with hostMemory if provided
     * @return image
     ...
     * @throws CLException
     */
    cl::Image3D createImage3D(const cl::ImageFormat& format, size_t width, size_t height, size_t depth, void* hostMemory = nullptr) const EXCEPT;

    /**
     * creates image2D of given format and dimensions filled with hostMemory if provided
     * @return image
     * @param format the format used in this image
     * @param width of image
     * @param height of image
     ...
     * @throws CLException
     */
    cl::Image2D createImage2D(const cl::ImageFormat& format, size_t width, size_t height, void* hostMemory = nullptr) const EXCEPT;

    /**
     * fills image with given clBuffer, make sure buffer size is equal or greater than
     * image size
    ...
    QVector<T> readImage(const cl::Image* image) const EXCEPT
    {
        ...
        size_t numElements = (imageSize % elementSize == 0) ? imageSize / elementSize : (imageSize / elementSize) +1;
        QVector<T> hostBuffer(numElements);
    ...
    /**
     * @return sampler with given filter and adressing mode normalized or not normalized
     * @param normalized whether sampler uses normalized coordinates
     * @param adressingMode CL_ADDRESS_NONE CL_ADDRESS_CLAMP_TO_EDGE CL_ADDRESS_CLAMP CL_ADDRESS_REPEAT
     * @param filterMode CL_FILTER_NEAREST CL_FILTER_LINEAR
     * @throws CLException
     */
    cl::Sampler createSampler(bool normalized, cl_addressing_mode adressingMode, cl_filter_mode filterMode) const EXCEPT;
 ...
 */
class VOXIECORESHARED_EXPORT IOException: public QException
{
private:
    QString message;
public:
...
VOXIECORESHARED_EXPORT QDebug operator<<(QDebug dbg, const IOException& ex);
