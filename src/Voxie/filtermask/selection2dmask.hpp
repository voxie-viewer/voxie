...
#include <Voxie/filtermask/ellipse.hpp>
#include <Voxie/filtermask/ellipseData.hpp>
#include <Voxie/filtermask/polygon.hpp>
...
namespace filter{
/**
 * Selection2DMask is class which every 2DFilter contains. This is for 2D masks. This class provides several methods for
 * creating a 2D Selection mask in the forms, Rectangle, Ellipse and Polygon. This Forms can also be deleted.
 * By changing the origin, this mask will be retained when the slice before and the new one are equal. For these
 * there are the slots translateOrigin whene origin is moved and rotate when origin is rotated.
 * @brief The Selection2DMask class is for all masks on 2D. This classes offers methods for creating masks, deleting
 * them and manipulating them when something changed from the origin. It also contains the drawing of the masks.
 */
...
public:
     ...
     * @return true if no shapes inside the mask, else it returns false when at least one shape exist.
     */
    bool isEmpty() {return rectangles.isEmpty() && ellipses.isEmpty() && polygons.isEmpty();}


    Selection2DMask(QObject* parent = nullptr) : QObject(parent), mutex(QMutex::Recursive) {}


 //-----------CPU---------------------------------
    QPainterPath getPath();
    /**
     * @brief Returns all ellipse transformation matrixs where are created on this mask
    ...
    /**
     * @brief Returns all rectangle transformation matrixs where are created on this mask
...
//------------GPU---------------------------------
     ...
     */
    cl::Buffer getEllipseBuffer(voxie::opencl::CLInstance* instance);
     ...
     */
    cl::Buffer getRectangleBuffer(voxie::opencl::CLInstance* instance);
     ...
     */
    cl::Buffer getPolygonBuffer(voxie::opencl::CLInstance* instance);

    /**
     * @brief getPolygonBufferOffset creates cl Buffer with integers which describes the offset
     * for the polygon buffer.
     * @param instance the instance for creating the buffers.
     * @return cl Buffer as a offset.
     */
    cl::Buffer getPolygonBufferOffset(voxie::opencl::CLInstance* instance);
...
//------------------------------------------------
     ...
     */
    void addEllipse(qreal midPointX, qreal midPointY, qreal radiusX, qreal radiusY);
    ...
    /**
     * @brief Checks a given Point of Floats, if the point is inside the mask or outside. It only works on CPU
...
public slots:
    /**
     * @brief translateOrigin translates the masks in dircetin of origin translation.
    ...
    /**
     * @brief rotate rotates the masks along the direction which the images is rotated.
